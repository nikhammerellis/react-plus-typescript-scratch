...
const AppStateProvider: React.FC = ({ children }) => {
    ...
    //get cart from localstorage and set cart to it (should be first)
    useEffect(() => {
    ...
    }, []); //empty dependency list will make sure the callback is called only once after the initial render 

    //update cart in local storage if it has changed 
    useEffect(() => {
        window.localStorage.setItem('cart', JSON.stringify(state.cart))
    }, [state.cart]); //only fire if state.cart changes 
